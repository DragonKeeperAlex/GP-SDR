...
| P0 | Complete P25 RF/audio acceptance | HackRF `c5cb` now locks EBRCS at 774.45625 MHz at 10 MS/s, reports NAC `0x1F5` and system `0x1F1`, follows grants, and receives Phase 2 TDULC/FACCH/SACCH frames. Intelligible audio is still pending JMBE plus an unencrypted call. Pluto did not lock in its short check; RTL testing was blocked by its earlier USB disappearance. | Install JMBE through its licensed workflow, accept an unencrypted HackRF call, then obtain control identity, grants, and audio independently on RTL-SDR and Pluto; record receiver, rate, gain, frequency error, identifiers, grant count, and audio quality. |
