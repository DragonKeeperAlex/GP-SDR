...
September 17, Pi test18: the tuner status now reports effective receiver sample rate and gain state from live telemetry—HackRF shows LNA/VGA/amp; other hardware shows generic gain. The telemetry is post-normalization/calibration and makes fallback state visible rather than relying on requested form values. UI regression, JavaScript syntax/audio queue, application tests and vet passed. Test18 SHA-256 is `ced3f1c4e8ca8c6e98ec5ff282091c3c2fc15c633e04f30d5b440ff7af856123`; rollback is `/home/sdr/gpsdr-before-pi-test18-MSVD6l`. Pluto P25 relocked 774.45625 MHz with service active/no throttle. Driver readback and browser visual acceptance remain open.

September 17, Pi test17: receiver-specific control handling now neutralizes HackRF-only front-end values in Tuner/Live requests for RTL and Pluto, matching the UI visibility rules. This prevents a hidden stale amp/LNA/VGA/bias state from being passed to another receiver. Regression, JavaScript syntax/audio queue, application tests and vet passed. Test17 SHA-256 is `266e105032b9a3ee18f83459fa6b8e27f0f68709ffb0f0bc624c1bae47304dbd`; rollback is `/home/sdr/gpsdr-before-pi-test17-BEAl1v`. Pluto P25 relocked 774.45625 MHz, service is active and throttle remains clear. Full UI and applied-hardware-control acceptance remain open.

September 17, Pi test16: live P25 capture-rate/gain/amp changes now restart the profile on the receiver actually reported by P25 status, rather than a stale client dropdown selection. This prevents a running Pluto session from unexpectedly switching to an old HackRF/RTL choice. The source regression, JavaScript syntax/audio scheduling, application tests and vet pass. Test16 is installed with SHA-256 `1251368a469593c05a9914df85cea0e70b4a0c4bd0e862d15d95daa2f3852357`; rollback is `/home/sdr/gpsdr-before-pi-test16-oMA3Py`. Pluto OP25 relocked 774.45625 MHz, service is active and throttling remains `0x0`. No profile setting was changed in the live browser; direct click-through and listening acceptance remain open.

September 17, Pi test15: the web audio state now explicitly says when the stream is connected but waiting for voice and switches to receiving with the actual PCM rate only after a valid frame arrives. The existing 8 kHz P25 scheduling regression passes. On the installed Pi, a 25-second direct no-playback stream capture during traffic received 378 P25 frames / 7.56 seconds PCM. Test15 SHA-256 is `f8b04edc0c9766cc1f4196af0f2d77970baeb570801d26b681d961d79221ec2c`; rollback is `/home/sdr/gpsdr-before-pi-test15-Odnzxy`. Pluto locked 774.45625 MHz after install; throttling is `0x0`. No user audio was played and this remains short of subjective audio acceptance.
...
`Scripts/pi/gp-sdr-p25.conf` sets `GPSDR_P25_ENGINE=op25` on this Pi. OP25 is now also the application default for native HackRF and RTL-SDR assignments, not only Soapy receivers. The application accepts `auto` as an OP25 alias and `sdrtrunk` as an explicit compatibility fallback; invalid values fail clearly, and explicitly selecting SDRTrunk for Pluto still fails rather than silently choosing another engine. OP25 must already be installed. This configuration uses the existing direct UDP PCM bridge rather than nonexistent native sound hardware. The separate SDRTrunk PCM bridge remains open.
