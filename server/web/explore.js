...
(() => {
  ...
  async function selectRow(key) {
    ...
    try{const captures=await api('/api/explore/captures?key='+encodeURIComponent(key));if(id!==detailRequest)return;byID('explore-captures').innerHTML=captures.map(event=>`<details><summary>${e(new Date(event.startedAt).toLocaleString())} · ${e(event.modulation||'Unknown')} · ${Number(event.durationSeconds).toFixed(2)} s · ${e(event.analysisStatus||'Not analyzed')}</summary><p>${e(event.transcript||'No speech transcript')}</p><p>${e(event.analysis?.summary||'No model summary')}</p><pre>${e(JSON.stringify(event.decoderMessages||[],null,2))}</pre>${event.audioPath?`<audio controls preload="none" src="/api/audio?id=${encodeURIComponent(event.id)}&token=${encodeURIComponent(serverToken)}"></audio>`:''}</details>`).join('')||'No journal captures available.';}catch(error){if(id===detailRequest)byID('explore-captures').textContent=error.message;}
  }
