...
function renderSpectrumAnalyzer(){
  ...
  const jobs=analyzerJobs(),active=jobs.filter(job=>job.state==='running'||job.state==='stopping'),records=analyzerRecords(),repeated=$('#analyzer-repeated')?.checked!==false,visible=records.filter(record=>!repeated||record.hits>1),selectedDevices=[...new Set(jobs.map(job=>job.config?.deviceID).filter(Boolean))],start=jobs.length?Math.min(...jobs.map(job=>job.config.startHz||Infinity)):Number($('#analyzer-start').value)*1e6,end=jobs.length?Math.max(...jobs.map(job=>job.config.endHz||0)):Number($('#analyzer-end').value)*1e6;
  $('#analyzer-state').textContent=active.length?`${active.length} sweeping`:'Idle';$('#analyzer-state').className=`chip ${active.length?'ready':''}`;$('#analyzer-receivers').textContent=String(active.length||selectedDevices.length);$('#analyzer-peaks').textContent=String(records.length);$('#analyzer-checks').textContent=jobs.reduce((sum,job)=>sum+Number(job.progress?.checksCompleted||0),0).toLocaleString();$('#analyzer-passes').textContent=jobs.reduce((sum,job)=>sum+Number(job.progress?.passesCompleted||0),0).toLocaleString();$('#analyzer-range-label').textContent=Number.isFinite(start)&&end>start?`${formatFrequency(start)} – ${formatFrequency(end)} · maximum observed level retained`:'No sweep data';$('#analyzer-stop').disabled=!active.length;$('#analyzer-identify').disabled=!analyzerSelectedPeaks.size;$('#analyzer-save').disabled=!records.length;
  const layout=$('#analyzer-layout').value,groups=layout==='separate'?selectedDevices.map(deviceID=>({id:deviceID,label:receiverLabel(state.devices.find(item=>item.id===deviceID)||{name:deviceID}),records:records.filter(record=>(record.deviceIDs||[]).includes(deviceID))})):[{id:'combined',label:'All assigned receivers',records}];root.className=groups.length?'analyzer-graphs':'analyzer-graphs empty-state compact';root.innerHTML=groups.length?groups.map((group,index)=>`<article class="analyzer-graph"><header><strong>${escapeHTML(group.label)}</strong><small>${group.records.length} sampled frequencies · max hold</small></header><canvas data-analyzer-graph="${index}" height="300"></canvas></article>`).join(''):'Start a sweep to build the wide spectrum.';groups.forEach((group,index)=>drawAnalyzerTrace(root.querySelector(`[data-analyzer-graph="${index}"]`),group.records,start,end));
  $('#analyzer-peak-list').innerHTML=visible.slice().sort((a,b)=>Number(b.strongestDBFS)-Number(a.strongestDBFS)).slice(0,500).map(record=>{const key=String(record.frequencyHz);return `<tr class="analyzer-peak-row"><td><input type="checkbox" data-analyzer-peak="${key}" ${analyzerSelectedPeaks.has(key)?'checked':''} aria-label="Select ${formatFrequency(record.frequencyHz)}"></td><td>${formatFrequency(record.frequencyHz)}</td><td>${Number(record.strongestDBFS).toFixed(1)} dBFS</td><td>${Number(record.noiseDBFS).toFixed(1)} dBFS</td><td>${record.hits}</td><td>${(record.deviceIDs||[]).length}</td><td>${timeAgo(record.lastSeen)}</td></tr>`;}).join('')||'<tr><td colspan="7">No retained peaks yet.</td></tr>';
...
for(const id of ['analyzer-layout','analyzer-repeated'])$('#'+id).addEventListener('change',renderSpectrumAnalyzer);
$('#analyzer-peak-list').addEventListener('change',event=>{const input=event.target.closest('[data-analyzer-peak]');if(!input)return;if(input.checked)analyzerSelectedPeaks.add(input.dataset.analyzerPeak);else analyzerSelectedPeaks.delete(input.dataset.analyzerPeak);$('#analyzer-identify').disabled=!analyzerSelectedPeaks.size;});
$('#analyzer-stop').addEventListener('click',async()=>{try{const active=analyzerJobs().filter(job=>job.state==='running'||job.state==='stopping');await Promise.all(active.map(job=>api('/api/mapper/jobs/stop',{method:'POST',body:JSON.stringify({id:job.id})})));toast(`${active.length} spectrum ${active.length===1?'sweep':'sweeps'} stopping`);await refreshAll();}catch(error){toast(error.message,true);}});
$('#analyzer-save').addEventListener('click',saveAnalyzerCSV);
$('#analyzer-clear').addEventListener('click',async()=>{const jobs=analyzerJobs();if(!jobs.length)return toast('Spectrum display is already clear');if(!await confirmAction({title:'Clear Spectrum Analyzer display?',message:'Reset the accumulated panorama and peak list? Underlying Mapper results stay available on the Results page.',confirmLabel:'Clear display'}))return;analyzerIgnoredJobIDs=new Set([...analyzerIgnoredJobIDs,...jobs.map(job=>job.id)]);localStorage.setItem('gpsdr-analyzer-ignored-jobs',JSON.stringify([...analyzerIgnoredJobIDs]));analyzerJobIDs=[];localStorage.setItem('gpsdr-analyzer-jobs','[]');analyzerSelectedPeaks.clear();renderSpectrumAnalyzer();toast('Spectrum Analyzer display cleared · Mapper results kept');});
$('#analyzer-identify').addEventListener('click',async()=>{try{await queueAnalyzerPeaks();}catch(error){toast(error.message,true);}});
...
async function pollSpectrum() {
	const mapperRunning=(state.view==='mapper'||state.view==='analyzer')&&mapperActiveJobs().length>0;
	if(!document.hidden && ((state.status?.running&&(state.view==='live'||state.view==='tuner'))||mapperRunning||state.view==='rfmonitor')){try{if(state.view==='rfmonitor'||state.view==='mapper'||state.view==='analyzer'){state.spectra=await api('/api/spectra?bins='+displayPrefs.detail);if(state.view==='rfmonitor')renderRFMonitor();else if(state.view==='mapper')renderMapperRF();else renderSpectrumAnalyzer();}else{state.spectrum=await api('/api/spectrum?bins='+displayPrefs.detail);renderTuner();drawSpectrum();drawWaterfall();}}catch(_){ }}
