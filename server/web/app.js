...
function mapperJobHTML(job){const p=job.progress||{},total=Number(p.totalTargets)||0,index=Number.isInteger(p.currentIndex)?p.currentIndex:-1,percent=total&&index>=0?Math.min(100,Math.max(0,((index+1)/total)*100)):0,running=job.state==='running'||job.state==='stopping',device=state.devices.find(item=>item.id===job.config.deviceID),eta=p.estimatedPassEndAt?compactDuration(Math.max(0,(new Date(p.estimatedPassEndAt)-Date.now())/1000)):'Calculating';return `<article class="mapper-job-card ${escapeHTML(job.state)}" data-job-id="${escapeHTML(job.id)}"><div class="mapper-job-head"><h3>${escapeHTML(job.name)}</h3><span class="chip ${running?'ready':job.state==='error'?'warning':''}">${escapeHTML(job.state||'idle')}</span></div><p>${escapeHTML(device?.name||job.config.deviceID)} · ${mapperWorkflowLabel(job.config.mode)}${job.config.mode==='discovery'?` · ${(job.config.startHz/1e6).toFixed(3)}–${(job.config.endHz/1e6).toFixed(3)} MHz`:' · found frequencies'}</p><strong class="mapper-job-frequency">${p.currentFrequencyHz?formatFrequency(p.currentFrequencyHz):'—'}</strong><div class="mapper-job-progress"><i style="width:${percent}%"></i></div><div class="mapper-job-stats"><span>${Number(p.checksCompleted||0).toLocaleString()} checks</span><span>${p.passesCompleted||0} passes</span><span>${running?`ETA ${eta}`:'Ready'}</span></div>${job.lastError?`<p class="mapper-job-error">${escapeHTML(job.lastError)}</p>`:''}<div class="mapper-job-actions"><button data-job-action="${running?'stop':'start'}">${running?'Stop':'Start'}</button><button data-job-action="edit" ${running?'disabled':''}>Edit</button><button data-job-action="duplicate">Duplicate</button><button data-job-action="export">Export</button><button data-job-action="delete" ${running?'disabled':''}>Delete</button></div></article>`;}
...
function saveDisplayPrefs(){localStorage.setItem('gpsdr-display-v2',JSON.stringify(displayPrefs));$('#display-smoothing-value').textContent=displayPrefs.smoothing+'%';$$('#live-waterfall,#waterfall').forEach(canvas=>{canvas.dataset.lastFrame='';canvas.width=0;});if(!displayPrefs.peakHold)[$('#spectrum'),$('#tuner-spectrum')].forEach(canvas=>spectrumPeaks.delete(canvas));drawSpectrum();drawWaterfall();}
...
$('#display-peak-reset').addEventListener('click',()=>{[$('#spectrum'),$('#tuner-spectrum')].forEach(canvas=>spectrumPeaks.delete(canvas));drawSpectrum();toast('Peak hold reset');});
renderTunerHistory();
applyMasterAudio(false);
saveDisplayPrefs();
