...
function renderHardware() {
  $('#device-grid').innerHTML = state.devices.length ? state.devices.map(device => `
    <article class="hardware-card"><div class="hardware-title"><i class="${device.healthWarning ? 'optional' : device.connected ? 'ready' : device.available ? 'optional' : ''}"></i><h3>${escapeHTML(device.name)}</h3></div>
      ...
      ${device.connected || device.available ? '' : setupActions(device.kind === 'HackRF' ? 'hackrf' : device.kind === 'RTL-SDR' ? 'rtlsdr' : device.kind === 'PlutoSDR' ? 'plutosdr' : 'soapysdr')}</article>`).join('') : '<div class="empty-state">No receiver drivers were detected. Install one below, then refresh.</div>';
