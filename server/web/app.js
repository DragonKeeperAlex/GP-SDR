...
function renderMapperProgress(){
  const m=state.mapper||{},jobs=m.jobs||[],active=jobs.filter(job=>job.progress?.running),selected=mapperSpectrumJob()||active[0]||jobs[0],p=selected?.progress||state.mapperProgress||{},records=m.records||[],running=active.length>0;
