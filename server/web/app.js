const state = {
  ...
  mapperPage: localStorage.getItem('gpsdr-mapper-page') || 'overview'
};
...
function confirmLearningSample(event){const dialog=$('#learning-dialog');$('#learning-event-id').value=event.id;$('#learning-frequency').value='';$('#learning-modulation').value=event.protocolName||event.modulation||'';$('#learning-protocol').value=event.protocolName||event.modulation||'';$('#learning-notes').value='';$('#learning-retain').checked=false;$('#learning-retain').disabled=!(event.iqPath||event.audioPath);dialog.showModal();}
function confirmMapperSample(record){const dialog=$('#learning-dialog');$('#learning-event-id').value='';$('#learning-frequency').value=record.frequencyHz;$('#learning-modulation').value=record.modulation||record.protocolName||'';$('#learning-protocol').value=record.protocolName||record.modulation||'';$('#learning-notes').value=record.detectionEvidence||record.verificationReason||'';$('#learning-retain').checked=false;$('#learning-retain').disabled=true;dialog.showModal();}
...
function receiverLabel(device, devices=state.devices) {
  const peers=devices.filter(item=>item.name===device.name&&item.kind===device.kind);
  if(peers.length<2)return device.name;
  const ordinal=peers.findIndex(item=>item.id===device.id)+1;
  return `${device.name} ${ordinal>0?ordinal:1}`;
}
...
function setMapperPage(page){
  state.mapperPage=['overview','discovery','identify','analysis','schedule','results'].includes(page)?page:'overview';localStorage.setItem('gpsdr-mapper-page',state.mapperPage);
  if(state.mapperPage==='discovery'||state.mapperPage==='identify'){const value=state.mapperPage==='discovery'?'discovery':'decipher',radio=$(`#mapper-workflow input[value="${value}"]`);if(radio&&!radio.checked){radio.checked=true;radio.dispatchEvent(new Event('change',{bubbles:true}));}}
  if(state.mapperPage==='schedule')document.querySelector('.mapper-compute-panel')?.setAttribute('open','');
  setView('mapper');
}
...
function analyzerJobs(){
  const jobs=state.mapper?.jobs||[],known=new Set(analyzerJobIDs);
  for(const job of jobs)if(String(job.name||'').startsWith('Spectrum analyzer ·')&&!analyzerIgnoredJobIDs.has(job.id))known.add(job.id);
  analyzerJobIDs=[...known];localStorage.setItem('gpsdr-analyzer-jobs',JSON.stringify(analyzerJobIDs));
  return jobs.filter(job=>known.has(job.id)&&!analyzerIgnoredJobIDs.has(job.id));
}

function analyzerRecords(){
  const ids=new Set(analyzerJobs().map(job=>job.id));
  return (state.mapper?.records||[]).filter(record=>(record.jobIDs||[]).some(id=>ids.has(id)));
}

function updateAnalyzerRange(){
  const full=$('#analyzer-full-range')?.checked,all=$('#analyzer-all')?.checked,select=$('#analyzer-device'),device=state.devices.find(item=>item.id===select?.value),radios=all?state.devices.filter(item=>item.connected&&item.available):device?[device]:[];
  if(select)select.disabled=!!all;
  for(const id of ['analyzer-start','analyzer-end'])if($('#'+id))$('#'+id).disabled=!!full;
  if(full&&radios.length){$('#analyzer-start').value=(Math.min(...radios.map(item=>item.frequencyMinimumHz||1e6))/1e6).toFixed(3);$('#analyzer-end').value=(Math.max(...radios.map(item=>item.frequencyMaximumHz||6e9))/1e6).toFixed(3);}
  const limit=radios.length?Math.min(...radios.map(item=>item.sampleRateLimit||20e6)):20e6;
  for(const option of $('#analyzer-rate')?.options||[])option.disabled=Number(option.value)>limit;
  if($('#analyzer-rate')?.selectedOptions[0]?.disabled)$('#analyzer-rate').value='0';
}

function drawAnalyzerTrace(canvas,records,start,end){
  if(!canvas||end<=start)return;const rect=canvas.getBoundingClientRect(),scale=devicePixelRatio||1,width=Math.max(320,rect.width||900),height=300,pad={left:48,right:12,top:12,bottom:26},plotWidth=width-pad.left-pad.right,plotHeight=height-pad.top-pad.bottom,floor=-120,ceiling=-15;canvas.width=width*scale;canvas.height=height*scale;const ctx=canvas.getContext('2d');ctx.scale(scale,scale);ctx.fillStyle='#05090e';ctx.fillRect(0,0,width,height);ctx.font='11px system-ui';for(let i=0;i<=5;i++){const db=ceiling-(ceiling-floor)*i/5,y=pad.top+plotHeight*i/5;ctx.strokeStyle='#202936';ctx.beginPath();ctx.moveTo(pad.left,y);ctx.lineTo(width-pad.right,y);ctx.stroke();ctx.fillStyle='#8290a2';ctx.fillText(`${Math.round(db)} dB`,4,y+4);}for(let i=0;i<=6;i++){const x=pad.left+plotWidth*i/6,frequency=start+(end-start)*i/6;ctx.strokeStyle='#18212c';ctx.beginPath();ctx.moveTo(x,pad.top);ctx.lineTo(x,pad.top+plotHeight);ctx.stroke();const label=formatFrequency(frequency),labelWidth=ctx.measureText(label).width;ctx.fillStyle='#8290a2';ctx.fillText(label,Math.max(pad.left,Math.min(width-pad.right-labelWidth,x-labelWidth/2)),height-7);}const bins=Math.max(256,Math.floor(plotWidth)),levels=new Float64Array(bins),noise=new Float64Array(bins);levels.fill(floor);noise.fill(floor);for(const record of records){if(record.frequencyHz<start||record.frequencyHz>end)continue;const index=Math.max(0,Math.min(bins-1,Math.round((record.frequencyHz-start)/(end-start)*(bins-1)))),noiseLevel=Number(record.noiseDBFS);if(Number.isFinite(noiseLevel))noise[index]=Math.max(noise[index],noiseLevel);const observed=record.hits>0?Number(record.strongestDBFS):noiseLevel;if(Number.isFinite(observed))levels[index]=Math.max(levels[index],observed);}let lastNoise=floor;for(let i=0;i<bins;i++){if(noise[i]>floor)lastNoise=noise[i];else noise[i]=lastNoise;if(levels[i]<=floor)levels[i]=noise[i];}let nextNoise=floor;for(let i=bins-1;i>=0;i--){if(noise[i]>floor)nextNoise=noise[i];else noise[i]=nextNoise;if(levels[i]<=floor)levels[i]=noise[i];}const yFor=value=>pad.top+plotHeight*(1-(Math.max(floor,Math.min(ceiling,value))-floor)/(ceiling-floor));ctx.beginPath();for(let i=0;i<bins;i++){const x=pad.left+plotWidth*i/(bins-1),y=yFor(noise[i]);i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.strokeStyle='#52708a';ctx.lineWidth=1;ctx.stroke();ctx.beginPath();for(let i=0;i<bins;i++){const x=pad.left+plotWidth*i/(bins-1),y=yFor(levels[i]);i?ctx.lineTo(x,y):ctx.moveTo(x,y);}ctx.strokeStyle='#63d7b5';ctx.lineWidth=1.5;ctx.stroke();ctx.lineTo(width-pad.right,pad.top+plotHeight);ctx.lineTo(pad.left,pad.top+plotHeight);ctx.closePath();ctx.fillStyle='rgba(77,214,170,.11)';ctx.fill();if(!records.length){ctx.fillStyle='#8290a2';ctx.font='13px system-ui';ctx.fillText('Waiting for sampled spectrum data',pad.left+12,pad.top+24);}}

function renderSpectrumAnalyzer(){
  const root=$('#analyzer-graphs');if(!root)return;const connected=state.devices.filter(item=>item.connected&&item.available),select=$('#analyzer-device'),previous=select.value,signature=connected.map(item=>item.id).join();if(select.dataset.signature!==signature){select.innerHTML=connected.map(item=>`<option value="${escapeHTML(item.id)}">${escapeHTML(receiverLabel(item,connected))}</option>`).join('')||'<option value="">No receiver</option>';select.dataset.signature=signature;if(connected.some(item=>item.id===previous))select.value=previous;updateAnalyzerRange();}
  const jobs=analyzerJobs(),active=jobs.filter(job=>job.state==='running'||job.state==='stopping'),records=analyzerRecords(),selectedDevices=[...new Set(jobs.map(job=>job.config?.deviceID).filter(Boolean))],start=jobs.length?Math.min(...jobs.map(job=>job.config.startHz||Infinity)):Number($('#analyzer-start').value)*1e6,end=jobs.length?Math.max(...jobs.map(job=>job.config.endHz||0)):Number($('#analyzer-end').value)*1e6;
  $('#analyzer-state').textContent=active.length?`${active.length} sweeping`:'Idle';$('#analyzer-state').className=`chip ${active.length?'ready':''}`;$('#analyzer-receivers').textContent=String(active.length||selectedDevices.length);$('#analyzer-peaks').textContent=String(records.length);$('#analyzer-checks').textContent=jobs.reduce((sum,job)=>sum+Number(job.progress?.checksCompleted||0),0).toLocaleString();$('#analyzer-passes').textContent=jobs.reduce((sum,job)=>sum+Number(job.progress?.passesCompleted||0),0).toLocaleString();$('#analyzer-range-label').textContent=Number.isFinite(start)&&end>start?`${formatFrequency(start)} – ${formatFrequency(end)} · maximum observed level retained`:'No sweep data';$('#analyzer-stop').disabled=!active.length;$('#analyzer-save').disabled=!records.length;
  root.className=records.length?'analyzer-graphs':'analyzer-graphs empty-state compact';root.innerHTML=records.length?`<article class="analyzer-graph"><header><strong>${selectedDevices.length>1?'Combined receivers':escapeHTML(receiverLabel(state.devices.find(item=>item.id===selectedDevices[0])||{name:'Spectrum'}))}</strong><small>${records.length} frequency bins · max hold</small></header><canvas data-analyzer-graph="0" height="420"></canvas></article>`:'Start a sweep to build the wide spectrum.';if(records.length)drawAnalyzerTrace(root.querySelector('[data-analyzer-graph="0"]'),records,start,end);
  const live=$('#analyzer-live-grid'),snapshots=(state.spectra||[]).filter(snapshot=>selectedDevices.includes(snapshot.deviceID));live.className=snapshots.length?'rf-monitor-grid':'rf-monitor-grid empty-state compact';live.innerHTML=snapshots.length?snapshots.map((snapshot,index)=>`<article class="rf-monitor-card"><div><strong>${escapeHTML(receiverLabel(state.devices.find(item=>item.id===snapshot.deviceID)||{name:snapshot.deviceID}))}</strong><small>${formatFrequency(snapshot.startFrequencyHz)} – ${formatFrequency(snapshot.endFrequencyHz)} · ${(snapshot.sampleRateHz/1e6).toFixed(2)} MS/s</small></div><canvas data-analyzer-live="${index}" height="170"></canvas><div class="frequency-axis"><span>${formatFrequency(snapshot.startFrequencyHz)}</span><span>${formatFrequency(snapshot.centerFrequencyHz)}</span><span>${formatFrequency(snapshot.endFrequencyHz)}</span></div></article>`).join(''):'Live slices appear while assigned receivers sweep.';snapshots.forEach((snapshot,index)=>drawReceiverSpectrum(live.querySelector(`[data-analyzer-live="${index}"]`),snapshot));
}

function analyzerDraft(device,startHz,endHz,name){return {id:'',name,config:{mode:'discovery',preferredMode:'auto',preferredDecoder:'auto',deviceID:device.id,useAllReceivers:false,startHz,endHz,stepHz:Number($('#analyzer-step').value),detectionBandwidthHz:Number($('#analyzer-bandwidth').value)||0,dwellMilliseconds:100,sampleRateHz:Number($('#analyzer-rate').value)||0,concurrentChannels:512,gainMode:'auto',ampMode:'auto',sensitivity:'auto',noiseMarginDB:Number($('#analyzer-margin').value)||8,capturePolicy:'channel',rejectedIQPolicy:'delete',transcribe:false,analysisPolicy:'manual',identifyMinimumHits:2,identifyHitSource:'discovery'}};}
async function startSpectrumAnalyzer(){const connected=state.devices.filter(item=>item.connected&&item.available),all=$('#analyzer-all').checked,chosen=all?connected:connected.filter(item=>item.id===$('#analyzer-device').value);if(!chosen.length)throw new Error('Connect and select a receiver first');const full=$('#analyzer-full-range').checked,requestedStart=Number($('#analyzer-start').value)*1e6,requestedEnd=Number($('#analyzer-end').value)*1e6;if(!full&&(!Number.isFinite(requestedStart)||!Number.isFinite(requestedEnd)||requestedEnd<=requestedStart))throw new Error('Enter a valid start and end frequency');const stamp=new Date().toLocaleString(),started=[];for(const device of chosen){const startHz=full?device.frequencyMinimumHz:Math.max(requestedStart,device.frequencyMinimumHz||requestedStart),endHz=full?device.frequencyMaximumHz:Math.min(requestedEnd,device.frequencyMaximumHz||requestedEnd);if(!(endHz>startHz))continue;const draft=analyzerDraft(device,startHz,endHz,`Spectrum analyzer · ${stamp} · ${receiverLabel(device,connected)}`),job=await api('/api/mapper/jobs',{method:'POST',body:JSON.stringify(draft)});await api('/api/mapper/jobs/start',{method:'POST',body:JSON.stringify({id:job.id})});started.push(job.id);}if(!started.length)throw new Error('The selected range is outside the connected receivers’ tuning limits');analyzerJobIDs=[...new Set([...analyzerJobIDs,...started])];localStorage.setItem('gpsdr-analyzer-jobs',JSON.stringify(analyzerJobIDs));toast(`${started.length} spectrum ${started.length===1?'sweep':'sweeps'} started`);await refreshAll();}
async function queueAnalyzerPeaks(){const records=analyzerRecords().filter(record=>analyzerSelectedPeaks.has(String(record.frequencyHz))).slice(0,100);if(!records.length)return;for(const record of records){const deviceID=(record.deviceIDs||[]).find(id=>state.devices.some(device=>device.id===id&&device.connected&&device.available))||$('#analyzer-device').value;await api('/api/mapper/jobs',{method:'POST',body:JSON.stringify({id:'',name:`Spectrum peak Identify · ${(record.frequencyHz/1e6).toFixed(6)} MHz`,config:{mode:'decipher',preferredMode:'auto',preferredDecoder:'auto',deviceID,startHz:record.frequencyHz,endHz:record.frequencyHz,stepHz:12500,decipherListenSeconds:60,sampleRateHz:0,concurrentChannels:1,gainMode:'auto',ampMode:'auto',sensitivity:'auto',noiseMarginDB:Number($('#analyzer-margin').value)||8,identifyMinimumHits:1,identifyHitSource:'all',identifyMinimumOccupancy:0,capturePolicy:'channel',rejectedIQPolicy:'delete',transcribe:true,analysisPolicy:'live'}})});}analyzerSelectedPeaks.clear();toast(`${records.length} ${records.length===1?'peak':'peaks'} added to the Identify queue`);await refreshAll();}
function saveAnalyzerCSV(){const rows=analyzerRecords();if(!rows.length)return;const cells=[['frequency_mhz','strongest_dbfs','noise_dbfs','snr_db','hits','checks','occupancy_percent','receiver_ids','first_seen','last_seen'],...rows.map(record=>[(record.frequencyHz/1e6).toFixed(6),record.strongestDBFS,record.noiseDBFS,(record.strongestDBFS-record.noiseDBFS).toFixed(1),record.hits,record.checks,(record.occupancy*100).toFixed(2),(record.deviceIDs||[]).join('|'),record.firstSeen,record.lastSeen])],csv=cells.map(row=>row.map(value=>`"${String(value??'').replaceAll('"','""')}"`).join(',')).join('\n'),url=URL.createObjectURL(new Blob([csv],{type:'text/csv'})),link=document.createElement('a');link.href=url;link.download=`gp-sdr-spectrum-${new Date().toISOString().replaceAll(':','-')}.csv`;link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}

let analyzerStatus={running:false,binsDBFS:[],startHz:0,endHz:0,sweeps:0,slices:0,deviceIDs:[]},analyzerView={startHz:0,endHz:0};
function analyzerVisibleRange(){const start=analyzerStatus.startHz||Number($('#analyzer-start').value)*1e6,end=analyzerStatus.endHz||Number($('#analyzer-end').value)*1e6;return analyzerView.endHz>analyzerView.startHz?analyzerView:{startHz:start,endHz:end};}
function analyzerDisplayRecords(){const bins=analyzerStatus.binsDBFS||[],fullStart=analyzerStatus.startHz,fullEnd=analyzerStatus.endHz;if(!bins.length||fullEnd<=fullStart)return[];return bins.map((level,index)=>({frequencyHz:fullStart+(fullEnd-fullStart)*index/Math.max(1,bins.length-1),strongestDBFS:Number(level),noiseDBFS:Number(level),hits:1}));}
function renderSpectrumAnalyzer(){
  const root=$('#analyzer-graphs');if(!root)return;const connected=state.devices.filter(item=>item.connected&&item.available),select=$('#analyzer-device'),previous=select.value,signature=connected.map(item=>item.id).join();if(select.dataset.signature!==signature){select.innerHTML=connected.map(item=>`<option value="${escapeHTML(item.id)}">${escapeHTML(receiverLabel(item,connected))}</option>`).join('')||'<option value="">No receiver</option>';select.dataset.signature=signature;if(connected.some(item=>item.id===previous))select.value=previous;updateAnalyzerRange();}
...
function renderDeferredAnalysis(){
  const status=state.analysisStatus||{},active=status.active||[],current=active[0]||status.current||{},done=Number(status.completed||0)+Number(status.failed||0),total=Number(status.total||0),progress=total?Math.min(100,100*done/total):0;
  const set=(id,value)=>{const el=$(id);if(el)el.textContent=value;};
  const stateChip=$('#analysis-state');if(stateChip){stateChip.textContent=status.running?'Running':status.pending?'Ready':'Idle';stateChip.className=`chip ${status.running?'ready':''}`;}
  set('#analysis-pending',String(status.pending||0));set('#analysis-file-progress',total?`${done} of ${total} files processed`:(status.pending?'Ready to process':'No work queued'));
  set('#analysis-groups',String(status.groups||0));set('#analysis-group-progress',status.groups?`${status.groupsDone||0} of ${status.groups} groups concluded`:'Grouped by frequency + receive area');
  set('#analysis-completed',String(status.completed||0));set('#analysis-failed',`${status.failed||0} failed`);set('#analysis-elapsed',status.startedAt?compactDuration(status.elapsedSeconds||0):'—');set('#analysis-eta',status.etaSeconds!=null?`ETA ${compactDuration(status.etaSeconds)}`:'ETA —');
  const bar=$('#analysis-progress-bar');if(bar)bar.style.width=`${progress}%`;
  set('#analysis-frequency',current.frequencyHz?formatFrequency(current.frequencyHz):'—');set('#analysis-location',current.location||'Waiting for work');set('#analysis-stage',current.stage||'Idle');set('#analysis-current-file',current.files?`File ${current.file||0} of ${current.files} · ${current.eventID||''}`:'—');
  const activeList=$('#analysis-active-list');if(activeList)activeList.innerHTML=active.length?active.map(item=>`<div class="analysis-active-card"><span>${item.files?`${item.file||0}/${item.files}`:''}</span><strong>${escapeHTML(formatFrequency(item.frequencyHz))}</strong><small>${escapeHTML(item.stage||'working')} · ${escapeHTML(item.location||'location not recorded')}</small></div>`).join(''):'<div class="analysis-empty">No frequency groups are active.</div>';
  const concurrency=$('#analysis-concurrency');if(concurrency){concurrency.disabled=!!status.running;concurrency.title=status.running?`${status.concurrency||1} frequency groups are running in parallel`:'Frequency/location groups processed at the same time. Auto uses available CPU, capped at 8.';}
  for(const id of ['#analysis-run','#mapper-analysis-run']){const button=$(id);if(button)button.disabled=!!status.running||!status.pending;}for(const id of ['#analysis-stop','#mapper-analysis-stop']){const button=$(id);if(button)button.disabled=!status.running;}
  const consoleEl=$('#analysis-console'),entries=(status.log||[]).slice(analysisLogFloor);if(consoleEl){consoleEl.innerHTML=entries.length?entries.map(entry=>`<div class="analysis-log-line ${escapeHTML(entry.level||'info')}"><span class="time">${escapeHTML(new Date(entry.at).toLocaleTimeString())}</span><span>${entry.frequencyHz?escapeHTML(formatFrequency(entry.frequencyHz)):'—'}</span><span class="stage">${escapeHTML(entry.stage||'')}</span><span class="message">${escapeHTML(entry.message||'')}</span></div>`).join(''):`<div class="analysis-empty">${status.running?'Waiting for the next analysis update…':'Start analysis to process stored captures.'}</div>`;if(status.running)consoleEl.scrollTop=consoleEl.scrollHeight;}
}
...
function renderMapper(){
  ...
  const m=state.mapper||{},jobs=m.jobs||[],jobNames=new Map(jobs.map(job=>[job.id,job.name])),deviceNames=new Map(state.devices.map(device=>[device.id,device.name]));
	const deferred=state.analysisStatus||{},analysisText=$('#mapper-analysis-status');if(analysisText)analysisText.textContent=deferred.running?`Computing · ${deferred.pending||0} queued · ${deferred.completed||0} complete`:(deferred.pending?`${deferred.pending} captures queued for compute`:'No deferred compute queued');if($('#mapper-analysis-run'))$('#mapper-analysis-run').disabled=!!deferred.running||!deferred.pending;if($('#mapper-analysis-stop'))$('#mapper-analysis-stop').disabled=!deferred.running;
...
function renderMapperRF(){
  const active=mapperActiveJobs(),deviceIDs=new Set(active.map(job=>job.config?.deviceID).filter(Boolean)),items=(state.spectra||[]).filter(snapshot=>deviceIDs.has(snapshot.deviceID)),root=$('#mapper-rf-grid'),badge=$('#mapper-rf-state');if(!root||!badge)return;
  badge.textContent=active.length?`${items.length}/${active.length} live`:'Waiting';badge.className=`chip ${active.length?'ready':''}`;
  $('#mapper-rf-detail').textContent=active.length?'Every active Mapper receiver is shown at the same time':'Start a Mapper job to watch its receivers';
  root.className=items.length?'rf-monitor-grid':'rf-monitor-grid empty-state compact';
  root.innerHTML=items.length?items.map((snapshot,index)=>{const device=state.devices.find(item=>item.id===snapshot.deviceID),deviceName=device?receiverLabel(device):snapshot.deviceID,job=active.find(item=>item.config?.deviceID===snapshot.deviceID),age=Math.max(0,(Date.now()-new Date(snapshot.capturedAt))/1000),frequencies=job?.progress?.currentFrequenciesHz||[];return `<article class="rf-monitor-card"><div><strong>${escapeHTML(job?.name||deviceName)}</strong><small>${escapeHTML(deviceName)} · ${(snapshot.sampleRateHz/1e6).toFixed(2)} MS/s · ${age<2?'live':`${Math.round(age)}s old`}</small></div><label>Spectrum · ${frequencies.length} channel${frequencies.length===1?'':'s'}</label><canvas data-mapper-spectrum-index="${index}" height="170"></canvas><label>Waterfall</label><canvas class="rf-waterfall" data-mapper-waterfall-index="${index}" height="120"></canvas><div class="frequency-axis"><span>${formatFrequency(snapshot.startFrequencyHz)}</span><span>${formatFrequency(snapshot.centerFrequencyHz)}</span><span>${formatFrequency(snapshot.endFrequencyHz)}</span></div></article>`;}).join(''):(active.length?'Waiting for the first IQ frame from each receiver.':'Start a Mapper job to watch its receivers.');
  items.forEach((snapshot,index)=>{drawReceiverSpectrum(root.querySelector(`[data-mapper-spectrum-index="${index}"]`),snapshot);drawReceiverWaterfall(root.querySelector(`[data-mapper-waterfall-index="${index}"]`),snapshot);});
}

function renderMapperProgress(){
  const m=state.mapper||{},jobs=m.jobs||[],active=jobs.filter(job=>job.progress?.running),records=m.records||[],running=active.length>0,progresses=active.map(job=>job.progress||{});
  const percents=progresses.map(p=>{const total=Number(p.totalTargets)||0,index=Number.isInteger(p.currentIndex)?p.currentIndex:-1,batches=Number(p.totalBatches)||0,batch=Number.isInteger(p.currentBatch)?p.currentBatch:-1;return batches&&batch>=0?100*(batch+1)/batches:total&&index>=0?100*(index+1)/total:0;}),percent=percents.length?percents.reduce((a,b)=>a+b,0)/percents.length:0;
  const channelCount=progresses.reduce((sum,p)=>sum+(p.currentFrequenciesHz?.length||0),0);
  $('#mapper-current-frequency').textContent=running?`${active.length} receiver${active.length===1?'':'s'} live`:'—';
  $('#mapper-current-detail').textContent=running?`${channelCount} channels in current batches · all jobs combined`:'Waiting to start';
  const list=$('#mapper-active-job-list');if(list)list.innerHTML=active.length?active.map(job=>{const p=job.progress||{},receiver=state.devices.find(device=>device.id===job.config?.deviceID),receiverName=receiver?receiverLabel(receiver):(job.config?.deviceID||'Receiver'),snapshot=(state.spectra||[]).find(item=>item.deviceID===job.config?.deviceID),frequencies=p.currentFrequenciesHz||[],batch=Number.isInteger(p.currentBatch)?p.currentBatch:0;return `<div class="mapper-active-row"><div><span>${escapeHTML(mapperWorkflowLabel(p.mode||job.config?.mode))}</span><strong>${escapeHTML(job.name||'Mapper job')}</strong><small>Pass ${(p.passesCompleted||0)+1} · batch ${batch+1} of ${p.totalBatches||'—'}</small></div><div><span>${escapeHTML(receiverName)}</span><strong>${snapshot?.sampleRateHz?`${(snapshot.sampleRateHz/1e6).toFixed(2)} MS/s`:'Preparing'}</strong><small>${snapshot?.centerFrequencyHz?`Centered ${formatFrequency(snapshot.centerFrequencyHz)} · ${escapeHTML(mapperTuningLabel(p.tuning||{}))}`:'Waiting for IQ'}</small></div><div><span>Channels being checked now</span><div class="mapper-channel-list">${frequencies.length?frequencies.map((frequency,index)=>`<span title="Software VFO ${index+1} of ${frequencies.length}">${formatFrequency(frequency)}</span>`).join(''):'<small>Preparing next batch</small>'}</div></div></div>`;}).join(''):'<div class="empty-state compact">Create or start a Mapper job.</div>';
  ...
  const lastActivity=progresses.map(p=>p.lastActivityAt).filter(Boolean).sort().at(-1),activityAge=lastActivity?timeAgo(lastActivity):'';$('#mapper-last-activity').textContent=activityAge?(activityAge==='now'?'Activity now':`Last activity ${activityAge} ago`):'No activity yet';
  const earliestStart=progresses.map(p=>p.startedAt).filter(Boolean).sort()[0];$('#mapper-elapsed').textContent=earliestStart?durationBetween(earliestStart,null):'—';
	const etaDates=progresses.map(p=>p.estimatedPassEndAt&&new Date(p.estimatedPassEndAt)).filter(Boolean),eta=etaDates.length?new Date(Math.max(...etaDates.map(value=>value.getTime()))):null,remaining=eta?Math.max(0,(eta.getTime()-Date.now())/1000):0;
  ...
  $('#mapper-channel-time').textContent=running?`${active.length} active job${active.length===1?'':'s'}`:'—';
...
function updateMapperWorkflow(){const workflow=mapperWorkflowValue(),identify=workflow==='decipher',adaptive=workflow==='adaptive',detail=$('#mapper-workflow-detail');$$('.mapper-discovery-control').forEach(control=>control.classList.toggle('hidden',identify));$$('.mapper-identify-control').forEach(control=>control.classList.toggle('hidden',!identify));$('#mapper-step').disabled=identify;$('#mapper-dwell-value').disabled=identify;$('#mapper-dwell-unit').disabled=identify;$('#mapper-listen-wrap').classList.toggle('hidden',!identify);$('#mapper-transcribe-wrap').classList.toggle('hidden',!(identify||adaptive));$('#mapper-start-caption').textContent=identify?'Filter start · MHz':'Start · MHz';$('#mapper-end-caption').textContent=identify?'Filter end · MHz':'End · MHz';$('#mapper-job-name').placeholder=identify?'Known-channel identification':adaptive?'General-purpose map':'UHF discovery';$('#mapper-start-button').textContent=identify?'Start identify':adaptive?'Start mapping':'Start discovery';detail.classList.toggle('identify',identify);detail.innerHTML=identify?'<strong>Identify</strong> focuses on one previously detected frequency at a time for maximum decoder, transcription, and classification accuracy.':adaptive?'<strong>Map</strong> repeatedly sweeps the selected range, detects activity, classifies and decodes it locally, records speech when enabled, and manages IQ evidence after analysis.':'<strong>Discovery</strong> prioritizes RF collection: one wide capture measures up to 512 channels, stores active evidence, and queues expensive analysis for later.';updateMapperGainControls();}

function applyMapperWorkflowDefaults(){const workflow=mapperWorkflowValue(),device=state.devices.find(item=>item.id===$('#mapper-device').value);$('#mapper-concurrent').value='0';if(workflow==='discovery'){$('#mapper-dwell-value').value='0.1';$('#mapper-dwell-unit').value='1000';$('#mapper-analysis-policy').value='manual';$('#mapper-transcribe').checked=false;$('#mapper-rate').value=device?.kind==='HackRF'?'20000000':'0';}else if(workflow==='decipher'){$('#mapper-analysis-policy').value='live';$('#mapper-transcribe').checked=true;$('#mapper-concurrent').value='1';}else{$('#mapper-analysis-policy').value='live';$('#mapper-transcribe').checked=true;$('#mapper-rate').value='0';}}

function updateMapperScheduleControls(){const enabled=$('#mapper-schedule-enabled').checked;$$('.mapper-schedule-control').forEach(control=>control.classList.toggle('hidden',!enabled));}

function setContextControls(selector,enabled){$$(selector).forEach(group=>{group.classList.toggle('auto-controlled',!enabled);group.querySelectorAll('input,select').forEach(control=>control.disabled=!enabled);});}
function updateMapperGainControls(){setContextControls('.mapper-manual-gain',$('#mapper-gain-mode').value==='manual');setContextControls('.mapper-manual-margin',$('#mapper-sensitivity').value==='manual');}
...
function renderStatus() {
  ...
  $('#storage-iq-archive').textContent=formatBytes(storage.iqArchiveBytes||0);
...
window.gpsdrNativeUpdateResult=result=>{state.appUpdate=result||{};renderAppUpdate();if(result?.state==='error')toast(result.message||'Update failed',true);};
...
function bandProfiles(){ return state.profiles.filter(profile => profile.channels?.length > 1 && !profile.p25Systems?.length); }
...
function renderBandMonitor(){
  ...
  profileSelect.innerHTML=profiles.map(profile=>`<option value="${escapeHTML(profile.id)}">${escapeHTML(profile.name)} · ${profile.channels.length}</option>`).join('');
  profileSelect.value=profiles.some(profile=>profile.id===prior)?prior:(profiles.find(profile=>profile.id==='be8e8ba2-ef4d-47f4-875f-f489bc8d894b')?.id||profiles[0]?.id||'');
  const connected=state.devices.filter(device=>device.connected&&device.available),oldDevice=deviceSelect.value;
  deviceSelect.innerHTML=connected.map(device=>`<option value="${escapeHTML(device.id)}">${escapeHTML(receiverLabel(device))}</option>`).join('');if(connected.some(device=>device.id===oldDevice))deviceSelect.value=oldDevice;
  const profile=profiles.find(item=>item.id===profileSelect.value),running=state.status?.running&&state.status.activeProfileID===profile?.id;
  const channels=running?state.mixer:(profile?.channels||[]).map(channel=>({id:channel.id,channel,muted:false,solo:false,volume:.8,pan:0,active:false,level:0}));
  const query=($('#band-search')?.value||'').trim().toLowerCase();
  const decorated=channels.map(item=>({item,event:latestEventForFrequency(item.channel.frequencyHz)})).filter(({item,event})=>{const code=event?.ctcssHz?`${Number(event.ctcssHz).toFixed(1)} hz`:'';return !query||[item.channel.name,shortFrequency(item.channel.frequencyHz),code].some(value=>String(value||'').toLowerCase().includes(query));}).sort((a,b)=>Number(b.item.active)-Number(a.item.active)||new Date(b.event?.startedAt||0)-new Date(a.event?.startedAt||0)||(a.item.channel.frequencyHz-b.item.channel.frequencyHz));
  $('#band-state').textContent=running?'Monitoring':'Idle';$('#band-state').className=`chip ${running?'ready':''}`;$('#band-channel-count').textContent=channels.length;$('#band-active-count').textContent=channels.filter(item=>item.active).length;
  const lastTone=decorated.find(entry=>entry.event?.ctcssHz)?.event?.ctcssHz;$('#band-last-code').textContent=lastTone?`${Number(lastTone).toFixed(1)} Hz`:'—';
  if(profile?.channels?.length){const values=profile.channels.map(channel=>channel.frequencyHz),span=Math.max(...values)-Math.min(...values);$('#band-capture-width').textContent=`${(span/1e6).toFixed(2)} MHz`;}
...
function renderEvents() {
  $('#events-body').innerHTML = state.events.length ? state.events.map(event => `
    ...
    <td>${event.audioPath ? `<button class="row-action play-event" data-event-id="${event.id}" title="Play or pause this recording">▶</button>` : ''}<button class="row-action learn-event" data-event-id="${event.id}" title="Confirm this event as a labeled example for local signal intelligence">✓</button></td></tr>`).join('')
...
function renderHardware() {
  const powerHat=state.status?.powerHat, powerCard=(powerHat?.available||powerHat?.error)?piPowerHatHTML(powerHat):'';
  $('#device-grid').innerHTML = state.devices.length||powerCard ? state.devices.map(device => `
    <article class="hardware-card"><div class="hardware-title"><i class="${device.healthWarning ? 'optional' : device.connected ? 'ready' : device.available ? 'optional' : ''}"></i><h3>${escapeHTML(device.name)}</h3></div>
      ...
      ${device.connected || device.available ? '' : setupActions(device.kind === 'HackRF' ? 'hackrf' : device.kind === 'RTL-SDR' ? 'rtlsdr' : device.kind === 'PlutoSDR' ? 'plutosdr' : 'soapysdr')}</article>`).join('')+powerCard : '<div class="empty-state">No receiver drivers were detected. Install one below, then refresh.</div>';
...
function piPowerHatHTML(power){if(!power.available)return `<article id="power-hat-card" data-power-error="true" class="hardware-card power-hat-card"><div class="hardware-title"><i class="warning"></i><h3>PiPower5 power HAT</h3></div><p>${escapeHTML(power.error||'Telemetry unavailable')}</p><footer><span>Readings unavailable · retrying automatically</span></footer></article>`;const battery=Math.max(0,Math.min(100,Number(power.batteryPercentage)||0)),source=power.powerSource||'Unknown';return `<article id="power-hat-card" class="hardware-card power-hat-card"><div class="hardware-title"><i class="ready"></i><h3>PiPower5 power HAT</h3></div><p id="power-hat-source">${escapeHTML(source)} power · ${power.charging?'charging':power.inputPluggedIn?'external input connected':'battery operation'}</p><div class="power-hat-battery"><span><i id="power-hat-level" style="width:${battery}%"></i></span><strong id="power-hat-percent">${battery.toFixed(0)}%</strong></div><dl class="power-hat-readings"><dt>Input</dt><dd id="power-hat-input">${Number(power.inputVoltage).toFixed(2)} V · ${Number(power.inputCurrent).toFixed(2)} A · ${Number(power.inputPower).toFixed(2)} W</dd><dt>Output</dt><dd id="power-hat-output">${Number(power.outputVoltage).toFixed(2)} V · ${Number(power.outputCurrent).toFixed(2)} A · ${Number(power.outputPower).toFixed(2)} W</dd><dt>Battery</dt><dd id="power-hat-battery">${Number(power.batteryVoltage).toFixed(2)} V · ${Number(power.batteryCurrent).toFixed(2)} A</dd></dl><footer><span>Pi power</span><span id="power-hat-updated">${power.updatedAt?`Updated ${timeAgo(power.updatedAt)} ago`:'Live telemetry'}</span></footer></article>`;}
function renderPiPowerHatStatus(){const power=state.status?.powerHat,card=$('#power-hat-card');if(!power?.available){if(power?.error){if(card)card.outerHTML=piPowerHatHTML(power);else if(state.view==='hardware')renderHardware();}else card?.remove();return;}if(card?.dataset.powerError){card.outerHTML=piPowerHatHTML(power);return;}if(!card){if(state.view==='hardware')renderHardware();return;}const battery=Math.max(0,Math.min(100,Number(power.batteryPercentage)||0));$('#power-hat-source').textContent=`${power.powerSource||'Unknown'} power · ${power.charging?'charging':power.inputPluggedIn?'external input connected':'battery operation'}`;$('#power-hat-level').style.width=`${battery}%`;$('#power-hat-percent').textContent=`${battery.toFixed(0)}%`;$('#power-hat-input').textContent=`${Number(power.inputVoltage).toFixed(2)} V · ${Number(power.inputCurrent).toFixed(2)} A · ${Number(power.inputPower).toFixed(2)} W`;$('#power-hat-output').textContent=`${Number(power.outputVoltage).toFixed(2)} V · ${Number(power.outputCurrent).toFixed(2)} A · ${Number(power.outputPower).toFixed(2)} W`;$('#power-hat-battery').textContent=`${Number(power.batteryVoltage).toFixed(2)} V · ${Number(power.batteryCurrent).toFixed(2)} A`;$('#power-hat-updated').textContent=power.updatedAt?`Updated ${timeAgo(power.updatedAt)} ago`:'Live telemetry';}
...
function renderTuner() {
  ...
  if (select.dataset.signature !== signature) {
    select.innerHTML = connected.length ? connected.map(device => `<option value="${escapeHTML(device.id)}">${escapeHTML(receiverLabel(device,connected))}</option>`).join('') : '<option value="">No receiver</option>';
  ...
  const tuning = state.status?.running && state.status?.activeProfileID === 'quick-tune';
	...
	if (tuning && tunedChannel) {
		if (document.activeElement !== $('#tuner-frequency') && $('#tuner-frequency').dataset.pending !== 'true') {
			...
			renderTunerDigits(Number(listenMHz));
		}
...
function renderDecoders() {
  ...
  if (detail.contains(document.activeElement) && document.activeElement.matches('input,select,textarea')) return;
...
function renderP25DecoderWorkspace() {
  ...
  const configuredRate = profile?.settings?.p25SampleRateHz || 0;
	...
	const displayedP25DeviceID=activeP25DeviceID||state.p25DeviceID;
	const p25Receiver=state.devices.find(item=>item.id===displayedP25DeviceID);
...
function p25RateOptions(configuredRate, deviceID=state.p25DeviceID) {
  const option = (rate, label) => `<option value="${rate}" ${configuredRate===rate?'selected':''}>${label}</option>`;
  const selected=state.devices.find(d=>d.id===deviceID);
	if(selected?.kind==='PlutoSDR')return option(0,'Auto · match receiver')+[1000000,2000000,2400000,5000000,10000000].filter(rate=>rate<=selected.sampleRateLimit).map(rate=>option(rate,`${rate/1e6} MS/s`)).join('');
...
function renderIntegrations() {
  ...
  const names = { dsp:'Live DSP', soapySDR:'SoapySDR', p25:'P25 trunking', transcription:'Transcription', radioReference:'RadioReference', localAI:'Local signal intelligence' };
  $('#integration-grid').innerHTML = Object.entries(names).map(([key, name]) => {
    ...
    const setupIDs = {soapySDR:'soapysdr',p25:'p25',transcription:'transcription',radioReference:'radioreference',localAI:'local-ai'};
...
function drawSpectrumCanvas(canvas) {
  ...
  for (let y = 0; y <= height; y += gridStepY) { ctx.beginPath(); ctx.moveTo(0,y); ctx.lineTo(width,y); ctx.stroke(); }
	...
	if(canvas.id!=='mapper-spectrum'&&displayPrefs.markers&&state.spectrum){const seen=new Set(),sourceProfiles=canvas.id==='band-spectrum'?state.profiles.filter(profile=>profile.id===$('#band-profile')?.value):state.profiles,channels=sourceProfiles.flatMap(profile=>profile.channels||[]).filter(channel=>channel.enabled!==false&&channel.frequencyHz>=state.spectrum.startFrequencyHz&&channel.frequencyHz<=state.spectrum.endFrequencyHz&&(!seen.has(Math.round(channel.frequencyHz))&&seen.add(Math.round(channel.frequencyHz)))).slice(0,canvas.id==='band-spectrum'?96:24);ctx.font='9px -apple-system,system-ui,sans-serif';for(const channel of channels){const x=(channel.frequencyHz-state.spectrum.startFrequencyHz)/(state.spectrum.endFrequencyHz-state.spectrum.startFrequencyHz)*width;ctx.beginPath();ctx.moveTo(x,0);ctx.lineTo(x,height);ctx.strokeStyle=canvas.id==='band-spectrum'?'rgba(73,213,170,.42)':'rgba(90,152,223,.28)';ctx.lineWidth=1;ctx.stroke();if(canvas.id==='band-spectrum'&&channels.length>36)continue;ctx.save();ctx.translate(x+3,10);ctx.rotate(Math.PI/2);ctx.fillStyle='rgba(151,164,180,.8)';ctx.fillText(String(channel.name||'Channel').slice(0,22),0,0);ctx.restore();}}
...
function drawSpectrum() {
  ...
  drawSpectrumCanvas($('#band-spectrum'));
}
...
function drawWaterfall() {
	for(const canvas of [$('#live-waterfall'),$('#waterfall'),$('#mapper-waterfall'),$('#band-waterfall')]) drawWaterfallCanvas(canvas);
}
...
function renderEditorRows() {
  ...
  $('#channels-editor').innerHTML = profile.channels.length ? profile.channels.map(channel => `
    <div class="editor-row channel" data-id="${channel.id}"><input data-key="name" value="${escapeHTML(channel.name)}" placeholder="Name" title="Channel name">
	  ...
	  <select data-key="decoder" title="Decoder for this channel">${profileDecoderOptions()}</select>
			<input class="channel-priority" data-key="priority" type="number" min="0" max="100" step="1" value="${Number(channel.priority)||0}" placeholder="Priority" title="Higher-priority active channels automatically duck lower-priority audio">
  ...
  $('#assignments-editor').innerHTML = profile.deviceAssignments.map(item => `
    <div class="editor-row assignment" data-id="${item.id}"><select data-key="role" title="Receiver role"><option value="automatic">Automatic</option><option value="discovery">Discovery sweep</option><option value="control">Trunk control</option><option value="voice">Trunk voice</option><option value="channelBank">Wideband bank</option><option value="parked">Parked channel</option></select>
      <select data-key="deviceID" title="Pin this role to a receiver"><option value="">Any compatible receiver</option>${state.devices.filter(d=>d.available).map(d=>`<option value="${d.id}">${escapeHTML(receiverLabel(d,state.devices))}</option>`).join('')}</select>
...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    const learn=event.target.closest('.learn-event');if(learn){const item=state.events.find(entry=>entry.id===learn.dataset.eventId);if(item)confirmLearningSample(item);return;}
...
function renderTunerDigits(value=Number($('#tuner-frequency').value||0)){
  const formatted=Math.max(.001,Number(value)||0).toFixed(6),decimal=formatted.indexOf('.');
  $('#tuner-readout').setAttribute('aria-label',`${formatted} megahertz`);
  $('#tuner-readout').innerHTML=[...formatted].map((character,index)=>{
    if(character==='.')return '<span class="tuner-digit-decimal">.</span>';
    const power=index<decimal?decimal-index-1:decimal-index;
    return `<span class="tuner-digit-control"><button type="button" data-tuner-digit="${power}" data-direction="1" title="Increase this digit" aria-label="Increase ${Math.pow(10,power)} megahertz">▲</button><b>${character}</b><button type="button" data-tuner-digit="${power}" data-direction="-1" title="Decrease this digit" aria-label="Decrease ${Math.pow(10,power)} megahertz">▼</button></span>`;
  }).join('');
}
function setTunerListenFrequency(frequencyHz,{apply=true,preserveCenter=false}={}){
  const mhz=Math.max(.001,Number(frequencyHz)/1e6);$('#tuner-frequency').value=mhz.toFixed(6);$('#tuner-frequency').dataset.pending='true';renderTunerDigits(mhz);
  if(preserveCenter&&state.spectrum?.centerFrequencyHz){$('#tuner-lock-center').checked=true;$('#tuner-hardware-center').value=(state.spectrum.centerFrequencyHz/1e6).toFixed(6);}
  drawSpectrum();if(apply&&state.status?.running&&state.status?.activeProfileID==='quick-tune')queueReceiverControls();
}
renderTunerDigits();
$('#tuner-frequency').addEventListener('input', event => renderTunerDigits(Number(event.target.value||0)));
$('#tuner-readout').addEventListener('click',event=>{const button=event.target.closest('[data-tuner-digit]');if(!button)return;const delta=Math.pow(10,Number(button.dataset.tunerDigit))*Number(button.dataset.direction)*1e6;setTunerListenFrequency(Number($('#tuner-frequency').value)*1e6+delta,{preserveCenter:true});});
...
$$('#view-tuner .radio-controls input, #view-tuner .radio-controls select, #tuner-gain, #tuner-rate').forEach(control=>control.addEventListener(control.type==='number'?'input':'change',queueReceiverControls));
...
$('#fixture-library-refresh').addEventListener('click',async()=>{const button=$('#fixture-library-refresh'),list=$('#fixture-library-list');button.disabled=true;list.textContent='Loading saved benchmarks…';try{const library=await api('/api/transmit/fixtures');list.textContent=(library.fixtures||[]).map(f=>`${f.kind} · ${new Date(f.createdAt).toLocaleString()} · ${f.durationSeconds.toFixed(2)} s\n${f.decodeStatus}\n${f.trainingEligibility||'Legacy fixture; receiver validation required'}\nIQ: ${f.iqPath}\nSHA-256: ${f.sha256}`).join('\n\n')||'No saved benchmarks.';if(library.invalid||library.truncated)list.textContent+=`\n\n${library.invalid} invalid manifests skipped${library.truncated?' · showing latest 200':''}`;}catch(error){list.textContent=error.message;toast(error.message,true);}finally{button.disabled=false;}});
...
$('#band-mute-all').addEventListener('click',async()=>{const mute=state.mixer.some(item=>!item.muted);try{await Promise.all(state.mixer.map(item=>api('/api/mixer',{method:'POST',body:JSON.stringify({id:item.id,muted:mute})})));await refreshAll();}catch(error){toast(error.message,true);}});
$('#mapper-full-auto').addEventListener('click',()=>{if(!state.devices.some(device=>device.connected&&device.available))return toast('Connect a receiver before starting Full auto',true);$('#mapper-job-name').value='Full auto mapping';$('#mapper-all-receivers').checked=true;const workflow=$('#mapper-workflow input[value="adaptive"]');workflow.checked=true;workflow.dispatchEvent(new Event('change',{bubbles:true}));$('#mapper-gain-mode').value='auto';$('#mapper-sensitivity').value='auto';$('#mapper-amp-mode').value='auto';$('#mapper-analysis-policy').value='after-job';$('#mapper-start-button').click();});
...
function collectMapperJob(){const workflow=mapperWorkflowValue(),start=Number($('#mapper-start').value)*1e6,end=Number($('#mapper-end').value)*1e6,allReceivers=$('#mapper-all-receivers')?.checked||false,deviceID=$('#mapper-device').value||state.devices.find(device=>device.connected&&device.available)?.id||'',step=Number($('#mapper-step').value)*1000,dwell=mapperDwellMilliseconds(),includeLocation=$('#mapper-location').checked;
  ...
  const decipherListenSeconds=Math.round(Number($('#mapper-listen-value').value)*Number($('#mapper-listen-unit').value));
	const scheduleEnabled=$('#mapper-schedule-enabled').checked,discoveryDurationSeconds=Math.round(Number($('#mapper-schedule-discovery-value').value)*Number($('#mapper-schedule-discovery-unit').value)),identifyDurationSeconds=Math.round(Number($('#mapper-schedule-identify-value').value)*Number($('#mapper-schedule-identify-unit').value));
  if(workflow==='decipher'&&(decipherListenSeconds<5||decipherListenSeconds>7*86400))throw new Error('Choose 5 seconds to 7 days per channel');
	if(!deviceID)throw new Error('Select a receiver or connect one before starting'); if(workflow!=='decipher'&&(!Number.isFinite(start)||!Number.isFinite(end)||end<start||step<=0))throw new Error('Enter a valid mapping range');if(workflow!=='decipher'&&(dwell<100||dwell>7*86400000))throw new Error('Choose 0.1 seconds to 7 days per channel'); if(includeLocation&&(!Number.isFinite(latitude)||!Number.isFinite(longitude)))throw new Error('Add a valid location or turn location tagging off');
	return {id:$('#mapper-job-id').value,name:$('#mapper-job-name').value.trim(),config:{mode:workflow,preferredMode:$('#mapper-mode').value,preferredDecoder:$('#mapper-decoder').value,deviceID,useAllReceivers:allReceivers,startHz:start,endHz:end,stepHz:step,dwellMilliseconds:dwell,sampleRateHz:Number($('#mapper-rate').value)||0,concurrentChannels:Number($('#mapper-concurrent').value)||0,gainMode:$('#mapper-gain-mode').value,gainDB:Number($('#mapper-gain').value),lnaGainDB:Number($('#mapper-lna').value),vgaGainDB:Number($('#mapper-vga').value),ampMode:$('#mapper-amp-mode').value,sensitivity:$('#mapper-sensitivity').value,noiseMarginDB:Number($('#mapper-noise-margin').value),decipherListenSeconds,identifyMinimumHits:Number($('#mapper-identify-min-hits').value)||1,identifyHitSource:$('#mapper-identify-hit-source').value,identifyMinimumOccupancy:Number($('#mapper-identify-occupancy').value)||0,identifySeenWithinHours:Number($('#mapper-identify-age').value)||0,identifyMaximumChannels:Number($('#mapper-identify-limit').value)||0,identifyOrder:$('#mapper-identify-order').value,capturePolicy:$('#mapper-capture-policy').value,rejectedIQPolicy:$('#mapper-rejected-iq').value,transcribe:$('#mapper-transcribe').checked,analysisPolicy:$('#mapper-analysis-policy').value,scheduleEnabled,discoveryDurationSeconds,identifyDurationSeconds,scheduleRepeat:$('#mapper-schedule-repeat').checked,includeLocation,locationPrecision:$('#mapper-location-precision').value,locationLabel:$('#mapper-location-label').value.trim(),latitude,longitude}};
}
async function saveMapperJob(start=false){try{const draft=collectMapperJob();if(start&&draft.config.useAllReceivers){state.mapper=await api('/api/mapper/jobs/start-all',{method:'POST',body:JSON.stringify(draft)});toast(`${draft.name||'Mapper'} started on all connected receivers`);await refreshAll();return null;}const job=await api('/api/mapper/jobs',{method:'POST',body:JSON.stringify(draft)});$('#mapper-job-id').value=job.id;if(start){state.mapper=await api('/api/mapper/jobs/start',{method:'POST',body:JSON.stringify({id:job.id})});toast(`${job.name} started`);}else toast(`${job.name} saved`);await refreshAll();return job;}catch(error){toast(error.message,true);return null;}}
function loadMapperJob(job,duplicate=false){const config=job?.config||{},suffix=duplicate?' copy':'';$('#mapper-job-id').value=duplicate?'':job?.id||'';$('#mapper-job-name').value=(job?.name||'')+suffix;if(config.deviceID)$('#mapper-device').value=config.deviceID;$('#mapper-all-receivers').checked=!!config.useAllReceivers;updateMapperReceiverSelection();setMapperWorkflow(config.mode||'adaptive');$('#mapper-mode').value=config.preferredMode||'auto';$('#mapper-decoder').value=config.preferredDecoder||'auto';if(config.startHz)$('#mapper-start').value=config.startHz/1e6;if(config.endHz)$('#mapper-end').value=config.endHz/1e6;if(config.stepHz)$('#mapper-step').value=config.stepHz/1000;setMapperDwellInput(config.dwellMilliseconds||500);$('#mapper-rate').value=String(config.sampleRateHz||0);$('#mapper-concurrent').value=String(config.concurrentChannels||0);$('#mapper-gain-mode').value=config.gainMode||'auto';$('#mapper-gain').value=String(config.gainDB??20);$('#mapper-lna').value=String(config.lnaGainDB??16);$('#mapper-vga').value=String(config.vgaGainDB??16);$('#mapper-amp-mode').value=config.ampMode||'auto';$('#mapper-sensitivity').value=config.sensitivity||'auto';$('#mapper-noise-margin').value=String(config.noiseMarginDB||6);setMapperListenInput(config.decipherListenSeconds||60);$('#mapper-identify-min-hits').value=String(config.identifyMinimumHits||2);$('#mapper-identify-hit-source').value=config.identifyHitSource||'discovery';$('#mapper-identify-occupancy').value=String(config.identifyMinimumOccupancy??.1);$('#mapper-identify-age').value=String(config.identifySeenWithinHours??168);$('#mapper-identify-limit').value=String(config.identifyMaximumChannels??50);$('#mapper-identify-order').value=config.identifyOrder||'hits';$('#mapper-capture-policy').value=config.capturePolicy||'channel';$('#mapper-rejected-iq').value=config.rejectedIQPolicy||'delete';$('#mapper-transcribe').checked=!!config.transcribe;$('#mapper-analysis-policy').value=config.analysisPolicy||'live';$('#mapper-schedule-enabled').checked=!!config.scheduleEnabled;$('#mapper-schedule-discovery-value').value=Math.max(1,(config.discoveryDurationSeconds||1800)/60);$('#mapper-schedule-discovery-unit').value='60';$('#mapper-schedule-identify-value').value=Math.max(1,(config.identifyDurationSeconds||1800)/60);$('#mapper-schedule-identify-unit').value='60';$('#mapper-schedule-repeat').checked=!!config.scheduleRepeat;$('#mapper-location').checked=!!config.includeLocation;$('#mapper-location-precision').value=config.locationPrecision||'approximate';$('#mapper-location-label').value=config.locationLabel||'';$('#mapper-latitude').value=config.latitude??'';$('#mapper-longitude').value=config.longitude??'';updateMapperScheduleControls();updateMapperWorkflow();$('#mapper-job-name').focus();}
function resetMapperJob(){loadMapperJob({config:{mode:'adaptive',preferredDecoder:'auto',startHz:10e6,endHz:6e9,stepHz:12500,dwellMilliseconds:500,decipherListenSeconds:60,sampleRateHz:0,concurrentChannels:0,gainMode:'auto',lnaGainDB:16,vgaGainDB:16,ampMode:'auto',sensitivity:'auto',noiseMarginDB:6,identifyMinimumHits:2,identifyHitSource:'discovery',identifyMinimumOccupancy:.1,identifySeenWithinHours:168,identifyMaximumChannels:50,identifyOrder:'hits',transcribe:true}},false);$('#mapper-job-id').value='';$('#mapper-all-receivers').checked=false;updateMapperReceiverSelection();}
$('#mapper-form').addEventListener('submit',async event=>{event.preventDefault();await saveMapperJob(true);});
$('#analyzer-form').addEventListener('submit',async event=>{event.preventDefault();try{await startSpectrumAnalyzer();}catch(error){toast(error.message,true);}});
for(const id of ['analyzer-device','analyzer-all','analyzer-full-range'])$('#'+id).addEventListener('change',()=>{updateAnalyzerRange();renderSpectrumAnalyzer();});
$('#analyzer-stop').addEventListener('click',async()=>{try{analyzerStatus=await api('/api/spectrum-analyzer/stop',{method:'POST',body:'{}'});renderSpectrumAnalyzer();toast('Spectrum sweep stopped');}catch(error){toast(error.message,true);}});
$('#analyzer-save').addEventListener('click',saveAnalyzerCSV);
$('#analyzer-clear').addEventListener('click',async()=>{analyzerStatus=await api('/api/spectrum-analyzer/clear',{method:'POST',body:'{}'});renderSpectrumAnalyzer();toast('Spectrum cleared');});
$('#analyzer-reset-view').addEventListener('click',()=>{analyzerView={startHz:analyzerStatus.startHz,endHz:analyzerStatus.endHz};renderSpectrumAnalyzer();});
$('#analyzer-graphs').addEventListener('wheel',event=>{if(!analyzerStatus.binsDBFS?.length)return;event.preventDefault();const range=analyzerVisibleRange(),span=range.endHz-range.startHz,rect=event.currentTarget.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width)),anchor=range.startHz+span*ratio,newSpan=Math.max((analyzerStatus.endHz-analyzerStatus.startHz)/32768,Math.min(analyzerStatus.endHz-analyzerStatus.startHz,span*(event.deltaY>0?1.35:.72)));let start=anchor-newSpan*ratio;start=Math.max(analyzerStatus.startHz,Math.min(analyzerStatus.endHz-newSpan,start));analyzerView={startHz:start,endHz:start+newSpan};renderSpectrumAnalyzer();},{passive:false});
let analyzerDrag=null;$('#analyzer-graphs').addEventListener('pointerdown',event=>{const range=analyzerVisibleRange();analyzerDrag={x:event.clientX,startHz:range.startHz,endHz:range.endHz};event.currentTarget.setPointerCapture(event.pointerId);});$('#analyzer-graphs').addEventListener('pointermove',event=>{const range=analyzerVisibleRange(),rect=event.currentTarget.getBoundingClientRect(),ratio=Math.max(0,Math.min(1,(event.clientX-rect.left)/rect.width)),frequency=range.startHz+(range.endHz-range.startHz)*ratio,index=Math.round((frequency-analyzerStatus.startHz)/(analyzerStatus.endHz-analyzerStatus.startHz)*Math.max(0,(analyzerStatus.binsDBFS?.length||1)-1)),level=analyzerStatus.binsDBFS?.[index];$('#analyzer-hover').textContent=`${formatFrequency(frequency)}${Number.isFinite(level)?` · ${Number(level).toFixed(1)} dBFS`:''}`;if(analyzerDrag){const span=analyzerDrag.endHz-analyzerDrag.startHz,shift=-(event.clientX-analyzerDrag.x)/Math.max(1,rect.width)*span;let start=Math.max(analyzerStatus.startHz,Math.min(analyzerStatus.endHz-span,analyzerDrag.startHz+shift));analyzerView={startHz:start,endHz:start+span};renderSpectrumAnalyzer();}});for(const type of ['pointerup','pointercancel','pointerleave'])$('#analyzer-graphs').addEventListener(type,()=>{analyzerDrag=null;});
$('#mapper-all-receivers').addEventListener('change',updateMapperReceiverSelection);updateMapperReceiverSelection();
$('#mapper-schedule-enabled').addEventListener('change',updateMapperScheduleControls);updateMapperScheduleControls();
$('#mapper-analysis-run').addEventListener('click',async()=>{try{const concurrency=Number($('#analysis-concurrency')?.value||0);state.analysisStatus=await api('/api/analysis/start',{method:'POST',body:JSON.stringify({concurrency})});toast(state.analysisStatus.pending?`Deferred compute started · ${state.analysisStatus.concurrency||1} parallel`:'No captures are waiting');await refreshAll();}catch(error){toast(error.message,true);}});
$('#mapper-analysis-stop').addEventListener('click',async()=>{try{state.analysisStatus=await api('/api/analysis/stop',{method:'POST',body:'{}'});toast('Deferred compute stopping');await refreshAll();}catch(error){toast(error.message,true);}});
$('#analysis-run').addEventListener('click',()=>$('#mapper-analysis-run').click());
$('#analysis-stop').addEventListener('click',()=>$('#mapper-analysis-stop').click());
$('#analysis-clear-log').addEventListener('click',()=>{analysisLogFloor=(state.analysisStatus?.log||[]).length;renderDeferredAnalysis();toast('Analysis log view cleared');});
$('#analysis-concurrency').value=localStorage.getItem('gpsdr-analysis-concurrency')||'0';$('#analysis-concurrency').addEventListener('change',event=>localStorage.setItem('gpsdr-analysis-concurrency',event.target.value));
...
$('#mapper-workflow').addEventListener('change',()=>{applyMapperWorkflowDefaults();updateMapperWorkflow();});
...
document.addEventListener('change', async event => {
	...
	const p25Rate=event.target.closest('#p25-live-rate,#p25-amp-mode,#p25-lna-gain,#p25-vga-gain');if(p25Rate){const profile=state.profiles.find(item=>item.id===(state.p25ProfileID||state.p25Status?.profileID||state.status?.activeProfileID||$('#p25-profile-choice')?.value));if(!profile)return toast('Start or select a P25 profile first',true);try{const wasRunning=state.status?.running&&state.status.activeProfileID===profile.id;const liveDeviceID=wasRunning?(state.p25Status?.receiverDeviceIDs||[])[0]:'';const deviceID=liveDeviceID||state.p25DeviceID;let updated=profile.builtIn?await api('/api/profiles/duplicate?id='+encodeURIComponent(profile.id),{method:'POST'}):structuredClone(profile);updated.settings||={};if(p25Rate.id==='p25-live-rate')updated.settings.p25SampleRateHz=Number(p25Rate.value)||0;else if(p25Rate.id==='p25-amp-mode')updated.settings.p25AmpMode=p25Rate.value;else updated.settings[p25Rate.id==='p25-lna-gain'?'p25LNAGainDB':'p25VGAGainDB']=p25Rate.value===''?null:Number(p25Rate.value);updated=await api('/api/profiles',{method:'POST',body:JSON.stringify(updated)});if(wasRunning){await api('/api/control/stop',{method:'POST',body:'{}'});await api('/api/control/start',{method:'POST',body:JSON.stringify({profileID:updated.id,deviceID:deviceID||undefined})});}state.selectedProfileID=state.p25ProfileID=updated.id;p25Rate.blur();toast('P25 receiver settings applied');await refreshAll();}catch(error){toast(error.message,true);}return;}
...
$('#learning-form').addEventListener('submit',async event=>{if(event.submitter?.value!=='confirm')return;event.preventDefault();try{const frequencyHz=Number($('#learning-frequency').value),body={modulation:$('#learning-modulation').value.trim(),protocol:$('#learning-protocol').value.trim(),notes:$('#learning-notes').value.trim()};if(frequencyHz>0)await api('/api/mapper/confirm',{method:'POST',body:JSON.stringify({...body,frequencyHz})});else await api('/api/local-ai/learning',{method:'POST',body:JSON.stringify({...body,eventID:$('#learning-event-id').value,retainCaptures:$('#learning-retain').checked})});$('#learning-dialog').close();$('#learning-frequency').value='';toast('Confirmed sample added to local learning');await refreshAll();}catch(error){toast(error.message,true);}});
resetMapperJob();
...
async function pollSpectrum() {
	...
	if(!document.hidden&&state.view==='analyzer'){try{analyzerStatus=await api('/api/spectrum-analyzer');renderSpectrumAnalyzer();}catch(_){}}
	else if(!document.hidden && ((state.status?.running&&(state.view==='live'||state.view==='tuner'||state.view==='band'))||mapperRunning||state.view==='rfmonitor')){try{if(state.view==='rfmonitor'||state.view==='mapper'){state.spectra=await api('/api/spectra?bins='+displayPrefs.detail);if(state.view==='rfmonitor')renderRFMonitor();else renderMapperRF();}else{state.spectrum=await api('/api/spectrum?bins='+displayPrefs.detail);if(state.view==='tuner')renderTuner();if(state.view==='band')renderBandMonitor();drawSpectrum();drawWaterfall();}}catch(_){ }}
	setTimeout(pollSpectrum,Math.max(16,1000/displayPrefs.fps));
}
pollSpectrum();

function saveDisplayPrefs(){localStorage.setItem('gpsdr-display-v2',JSON.stringify(displayPrefs));$('#display-smoothing-value').textContent=displayPrefs.smoothing+'%';$$('#live-waterfall,#waterfall,#mapper-waterfall,#band-waterfall').forEach(canvas=>{canvas.dataset.lastFrame='';canvas.width=0;});if(!displayPrefs.peakHold)[$('#spectrum'),$('#tuner-spectrum'),$('#mapper-spectrum'),$('#band-spectrum')].forEach(canvas=>spectrumPeaks.delete(canvas));drawSpectrum();drawWaterfall();}
...
function setVFOFromPointer(event){const point=frequencyAtPointer(event);if(!point)return;const active=state.status?.running&&state.status?.activeProfileID==='quick-tune';setTunerListenFrequency(point.frequencyHz,{apply:active,preserveCenter:active});toast(`${active?'Software VFO':'Tuner set to'} ${(point.frequencyHz/1e6).toFixed(6)} MHz${active?' · hardware center held':''}`);}
...
$('#band-spectrum').addEventListener('mousemove',event=>{const point=frequencyAtPointer(event);if(!point)return;spectrumCursors.set(event.currentTarget,point);const index=Math.min(state.spectrum.binsDBFS.length-1,Math.max(0,Math.round(point.fraction*(state.spectrum.binsDBFS.length-1)))),db=state.spectrum.binsDBFS[index];$('#band-spectrum-cursor').textContent=`${(point.frequencyHz/1e6).toFixed(6)} MHz · ${db.toFixed(1)} dBFS`;drawSpectrumCanvas(event.currentTarget);});
$('#band-spectrum').addEventListener('mouseleave',event=>{spectrumCursors.delete(event.currentTarget);$('#band-spectrum-cursor').textContent='Hover for frequency';drawSpectrumCanvas(event.currentTarget);});
$('#tuner-frequency').addEventListener('keydown',event=>{if(event.key!=='ArrowUp'&&event.key!=='ArrowDown')return;event.preventDefault();const direction=event.key==='ArrowUp'?1:-1,next=Math.max(.001,Number(event.currentTarget.value)+direction*Number($('#tuner-step').value));setTunerListenFrequency(next*1e6,{preserveCenter:true});});
...
$('#tuner-history').addEventListener('change',event=>{const item=tunerHistory.find(entry=>String(entry.frequencyHz)===event.target.value);if(!item)return;setTunerListenFrequency(item.frequencyHz,{apply:false});$('#tuner-mode').value=item.mode;$('#tuner-bandwidth').value=item.bandwidthHz/1e3;});
