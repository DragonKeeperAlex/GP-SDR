...
function renderStatus() {
  ...
  const storageForm=$('#storage-policy-form'),policy=storage.policy||{};if(storageForm&&!storageForm.contains(document.activeElement)){const gb=1024**3;$('#storage-auto-cleanup').checked=!!policy.autoCleanup;$('#storage-max-days').value=String(policy.maxCaptureDays??30);$('#storage-recording-cap').value=String(Math.round(Number(policy.recordingCapBytes||0)/gb));$('#storage-iq-cap').value=String(Math.round(Number(policy.iqCapBytes||0)/gb));}
...
$('#storage-policy-form').addEventListener('submit',async event=>{event.preventDefault();const gb=1024**3;try{const storage=await api('/api/storage/policy',{method:'PUT',body:JSON.stringify({autoCleanup:$('#storage-auto-cleanup').checked,maxCaptureDays:Number($('#storage-max-days').value),recordingCapBytes:Math.round(Number($('#storage-recording-cap').value)*gb),iqCapBytes:Math.round(Number($('#storage-iq-cap').value)*gb)})});state.status.storage=storage;renderStatus();toast('Storage limits saved');}catch(error){toast(error.message,true);}});
