...
function analyzerJobs(){
  ...
  for(const job of jobs)if(String(job.name||'').startsWith('Spectrum analyzer ·'))known.add(job.id);
  analyzerJobIDs=[...known];localStorage.setItem('gpsdr-analyzer-jobs',JSON.stringify(analyzerJobIDs));
  return jobs.filter(job=>known.has(job.id));
}
...
function drawAnalyzerTrace(canvas,records,start,end){
  if(!canvas||!records.length||end<=start)return;const rect=canvas.getBoundingClientRect(),scale=devicePixelRatio||1,width=Math.max(320,rect.width||900),height=240;canvas.width=width*scale;canvas.height=height*scale;const ctx=canvas.getContext('2d');ctx.scale(scale,scale);ctx.fillStyle='#070b10';ctx.fillRect(0,0,width,height);ctx.strokeStyle='#202936';ctx.lineWidth=1;for(let i=0;i<=4;i++){const y=i*height/4;ctx.beginPath();ctx.moveTo(0,y);ctx.lineTo(width,y);ctx.stroke();}const floor=-120,ceiling=-15;ctx.strokeStyle='#63d7b5';ctx.lineWidth=1.5;ctx.beginPath();let began=false;for(const record of [...records].sort((a,b)=>a.frequencyHz-b.frequencyHz)){if(record.frequencyHz<start||record.frequencyHz>end)continue;const x=(record.frequencyHz-start)/(end-start)*width,y=height-(Math.max(floor,Math.min(ceiling,Number(record.strongestDBFS)||floor))-floor)/(ceiling-floor)*height;if(!began){ctx.moveTo(x,height);ctx.lineTo(x,y);began=true}else ctx.lineTo(x,y);}if(began){ctx.stroke();ctx.lineTo(width,height);ctx.globalAlpha=.12;ctx.fillStyle='#63d7b5';ctx.fill();ctx.globalAlpha=1;}ctx.fillStyle='#8d9aad';ctx.font='11px system-ui';ctx.fillText(formatFrequency(start),7,height-7);const label=formatFrequency(end);ctx.fillText(label,width-ctx.measureText(label).width-7,height-7);}
...
function renderSpectrumAnalyzer(){
  ...
  const layout=$('#analyzer-layout').value,groups=layout==='separate'?selectedDevices.map(deviceID=>({id:deviceID,label:receiverLabel(state.devices.find(item=>item.id===deviceID)||{name:deviceID}),records:visible.filter(record=>(record.deviceIDs||[]).includes(deviceID))})):[{id:'combined',label:'All assigned receivers',records:visible}];root.className=groups.length?'analyzer-graphs':'analyzer-graphs empty-state compact';root.innerHTML=groups.length?groups.map((group,index)=>`<article class="analyzer-graph"><header><strong>${escapeHTML(group.label)}</strong><small>${group.records.length} retained peaks</small></header><canvas data-analyzer-graph="${index}" height="240"></canvas></article>`).join(''):'Start a sweep to accumulate peaks across the selected range.';groups.forEach((group,index)=>drawAnalyzerTrace(root.querySelector(`[data-analyzer-graph="${index}"]`),group.records,start,end));
