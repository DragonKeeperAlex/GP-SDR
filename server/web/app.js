...
$('#analyzer-clear').addEventListener('click',async()=>{const jobs=analyzerJobs();if(!jobs.length)return toast('Spectrum display is already clear');if(!await confirmAction({title:'Clear Spectrum Analyzer display?',message:'Reset the accumulated spectrum display?',confirmLabel:'Clear display'}))return;analyzerIgnoredJobIDs=new Set([...analyzerIgnoredJobIDs,...jobs.map(job=>job.id)]);localStorage.setItem('gpsdr-analyzer-ignored-jobs',JSON.stringify([...analyzerIgnoredJobIDs]));analyzerJobIDs=[];localStorage.setItem('gpsdr-analyzer-jobs','[]');renderSpectrumAnalyzer();toast('Spectrum Analyzer display cleared');});
