const state = {
  ...
  rangeSync: null, localDatabase: null, calibrations: [], mapper: null, mapperProgress: null, remoteReceivers: [],
...
async function refreshAll() {
  try {
    const [status, profiles, events, signals, devices, decoders, mixer, integrations, setup, p25Status, spectrum, rangeSync, localDatabase, calibrations, mapper, mapperProgress, remoteReceivers] = await Promise.all([
      ...
      api('/api/p25/status'), api('/api/spectrum'), api('/api/range-sync'), api('/api/local-database'), api('/api/calibrations'), api('/api/mapper'), api('/api/mapper/progress'), api('/api/remote-receivers')
    ]);
    Object.assign(state, { status, profiles: profiles || [], events: events || [], signals: signals || [], devices: devices || [], decoders: decoders || [], mixer: mixer || [], integrations, setup, p25Status, spectrum, rangeSync, localDatabase, calibrations: calibrations || [], mapper, mapperProgress, remoteReceivers: remoteReceivers || [] });
...
function render() {
  ...
  renderEvents(); renderProfiles(); renderHardware(); renderIntegrations(); renderRadioReferenceSettings(); renderRangeSync(); renderLocalDatabase(); renderTuner(); renderDecoders(); renderMapper(); renderMissingComponents(); drawSpectrum(); drawWaterfall();
}
...
function renderMapper(){
  ...
  const query=$('#mapper-filter-search').value.trim().toLowerCase(),resultState=$('#mapper-filter-state').value,now=Date.now();
  const records=allRecords.filter(record=>{const identified=!!(record.name||record.protocolName||record.identificationSource),haystack=[record.frequencyHz,formatFrequency(record.frequencyHz),record.name,record.protocolName,record.modulation,record.identificationSource,record.candidateDecoder,record.detectionEvidence,record.analysisSummary,record.lastTranscript,...(record.callsigns||[])].filter(Boolean).join(' ').toLowerCase();return (!jobFilter.value||(record.jobIDs||[]).includes(jobFilter.value))&&(!deviceFilter.value||(record.deviceIDs||[]).includes(deviceFilter.value))&&(!typeFilter.value||(record.protocolName||record.modulation)===typeFilter.value)&&(!query||haystack.includes(query))&&(!resultState||(resultState==='identified'&&identified)||(resultState==='unidentified'&&!identified)||(resultState==='recent'&&now-new Date(record.lastSeen).getTime()<=3600000)||(resultState==='frequent'&&Number(record.hits)>=10));});
  const sort=$('#mapper-sort').value;records.sort((a,b)=>sort==='frequency'?a.frequencyHz-b.frequencyHz:sort==='hits'?Number(b.hits)-Number(a.hits)||new Date(b.lastSeen)-new Date(a.lastSeen):sort==='occupancy'?Number(b.occupancy)-Number(a.occupancy)||Number(b.hits)-Number(a.hits):sort==='confidence'?Number(b.confidence)-Number(a.confidence)||new Date(b.lastSeen)-new Date(a.lastSeen):new Date(b.lastSeen)-new Date(a.lastSeen));
...
function mapperWorkflowLabel(mode){return mode==='decipher'?'Identify':'Discovery';}
function mapperWorkflowValue(){return $('#mapper-workflow input:checked')?.value||'discovery';}
function setMapperWorkflow(value){const normalized=value==='decipher'?'decipher':'discovery',input=$(`#mapper-workflow input[value="${normalized}"]`);if(input)input.checked=true;}
...
function mapperJobHTML(job){const p=job.progress||{},total=Number(p.totalTargets)||0,index=Number.isInteger(p.currentIndex)?p.currentIndex:-1,totalBatches=Number(p.totalBatches)||0,batch=Number.isInteger(p.currentBatch)?p.currentBatch:-1,percent=totalBatches&&batch>=0?Math.min(100,Math.max(0,((batch+1)/totalBatches)*100)):total&&index>=0?Math.min(100,Math.max(0,((index+1)/total)*100)):0,running=job.state==='running'||job.state==='stopping',device=state.devices.find(item=>item.id===job.config.deviceID),eta=p.estimatedPassEndAt?compactDuration(Math.max(0,(new Date(p.estimatedPassEndAt)-Date.now())/1000)):'Calculating',channels=p.monitoredChannels||job.config.concurrentChannels||(job.config.mode==='decipher'?4:16);return `<article class="mapper-job-card ${escapeHTML(job.state)}" data-job-id="${escapeHTML(job.id)}"><div class="mapper-job-head"><h3>${escapeHTML(job.name)}</h3><span class="chip ${running?'ready':job.state==='error'?'warning':''}">${escapeHTML(job.state||'idle')}</span></div><p>${escapeHTML(device?.name||job.config.deviceID)} · ${mapperWorkflowLabel(job.config.mode)}${job.config.mode==='discovery'?` · ${(job.config.startHz/1e6).toFixed(3)}–${(job.config.endHz/1e6).toFixed(3)} MHz`:' · found frequencies'} · up to ${channels} at once</p><strong class="mapper-job-frequency">${mapperBatchReadout(p)}</strong><div class="mapper-job-progress"><i style="width:${percent}%"></i></div><div class="mapper-job-stats"><span>${Number(p.checksCompleted||0).toLocaleString()} checks</span><span>${p.passesCompleted||0} passes</span><span>${running?`ETA ${eta}`:'Ready'}</span></div>${job.lastError?`<p class="mapper-job-error">${escapeHTML(job.lastError)}</p>`:''}<div class="mapper-job-actions"><button data-job-action="${running?'stop':'start'}">${running?'Stop':'Start'}</button><button data-job-action="edit" ${running?'disabled':''}>Edit</button><button data-job-action="duplicate">Duplicate</button><button data-job-action="export">Export</button><button data-job-action="delete" ${running?'disabled':''}>Delete</button></div></article>`;}
...
function renderMapperProgress(){
  ...
  const snapshot=state.spectrum;$('#mapper-capture').textContent=snapshot?.sampleRateHz?`${(snapshot.sampleRateHz/1e6).toFixed(2)} MS/s`:'—';$('#mapper-capture-detail').textContent=snapshot?.centerFrequencyHz?`${receiver?.name||selected?.config?.deviceID||'Receiver'} · centered ${formatFrequency(snapshot.centerFrequencyHz)}`:'No sampled band yet';
...
function updateMapperWorkflow(){const identify=mapperWorkflowValue()==='decipher',detail=$('#mapper-workflow-detail');$$('.mapper-discovery-control').forEach(control=>control.classList.toggle('hidden',identify));$('#mapper-step').disabled=identify;$('#mapper-dwell').disabled=identify;$('#mapper-listen-wrap').classList.toggle('hidden',!identify);$('#mapper-transcribe-wrap').classList.toggle('hidden',!identify);$('#mapper-start-caption').textContent=identify?'Filter start · MHz':'Start · MHz';$('#mapper-end-caption').textContent=identify?'Filter end · MHz':'End · MHz';$('#mapper-job-name').placeholder=identify?'Known-channel identification':'UHF discovery';$('#mapper-start-button').textContent=identify?'Start identify':'Start discovery';detail.classList.toggle('identify',identify);detail.innerHTML=identify?'<strong>Identify</strong> monitors nearby found frequencies from the same IQ capture, listens longer, and attempts classification, decoding, callsigns, and transcription. Auto uses four channels at once to limit CPU use.':'<strong>Discovery</strong> checks multiple frequency steps from each IQ capture and records active frequencies. Auto uses up to sixteen channels at once.';}
...
function renderDecoders() {
  ...
  const relevantProfiles = state.profiles.filter(profile => decoder.id === 'p25' ? (profile.p25Systems || []).length :
    ...
    profile.ranges.some(range => range.decoder === decoder.id));
...
function renderP25DecoderWorkspace() {
  ...
  return `<article class="panel p25-overview"><div class="p25-metrics"><div><span>Engine</span><strong>${escapeHTML(status.engine || 'Bundled')}</strong></div><div><span>Reception</span><strong>${escapeHTML(status.reception || status.state || 'setup')}</strong></div><div><span>Control channel</span><strong>${status.controlChannelHz ? formatFrequency(status.controlChannelHz) : 'Searching'}</strong>${status.controlChannelHz ? `<small>${status.controlSource === 'decoded' ? 'Decoded current' : 'Configured primary'}</small>` : ''}</div><div><span>Capture width</span><strong>${status.captureRateHz ? `${status.captureRateHz/1e6} MS/s` : 'Auto'}</strong></div><div><span>Talkgroups</span><strong>${talkgroups.length}</strong></div><div><span>Calibration</span><strong>${calibrated}/${connected.length}</strong></div></div><p class="hardware-detail">${escapeHTML(status.note || '')}${calibrated ? ' · Saved PPM, gain, and front-end calibration applied; P25 IQ tracking remains automatic.' : ' · Calibrate the receiver on the Hardware page for best results.'}</p>${voiceSetup}
    <div class="panel-head"><div><h2>Talkgroup mixer</h2><span>${active ? `${active} active · ` : ''}Mute and solo independently</span></div><div class="p25-mixer-tools"><label>Capture<select id="p25-live-rate" title="P25 receiver bandwidth; changing it restarts the active P25 profile"><option value="0" ${configuredRate===0?'selected':''}>Auto · 10</option><option value="5000000" ${configuredRate===5000000?'selected':''}>5 MS/s</option><option value="8000000" ${configuredRate===8000000?'selected':''}>8 MS/s</option><option value="10000000" ${configuredRate===10000000?'selected':''}>10 MS/s</option><option value="20000000" ${configuredRate===20000000?'selected':''}>20 MS/s</option></select></label><label>Order<select id="p25-order" title="Order calls by latest activity or total received"><option value="recent" ${state.p25Order === 'recent' ? 'selected' : ''}>Most recent</option><option value="heard" ${state.p25Order === 'heard' ? 'selected' : ''}>Most received</option></select></label><button class="decoder-mute-all icon-button" title="Mute or unmute every P25 talkgroup">M</button></div></div>
...
function renderEditorRows() {
  ...
  $('#ranges-editor').innerHTML = profile.ranges.length ? profile.ranges.map(range => `
    <div class="editor-row" data-id="${range.id}"><input data-key="name" value="${escapeHTML(range.name)}" placeholder="Name" title="Range name">
      ...
      <input data-key="decoder" value="${escapeHTML(range.decoder || '')}" placeholder="Decoder" title="Optional decoder used for activity in this range">
...
function collectEditor() {
  ...
  profile.ranges = $$('#ranges-editor .editor-row').map(row => ({
    ...
    dwellMilliseconds: 180, preferredMode: row.querySelector('[data-key="preferredMode"]').value, decoder:row.querySelector('[data-key="decoder"]').value.trim()||null, enabled: true
  }));
  profile.channels = $$('#channels-editor .editor-row').map(row => ({
    ...
    decoder: row.querySelector('[data-key="decoder"]').value.trim() || null, enabled: true, priority: Number(row.querySelector('[data-key="priority"]')?.value)||0
  }));
...
function collectMapperJob(){const workflow=mapperWorkflowValue(),start=Number($('#mapper-start').value)*1e6,end=Number($('#mapper-end').value)*1e6,deviceID=$('#mapper-device').value,step=Number($('#mapper-step').value)*1000,dwell=Number($('#mapper-dwell').value),includeLocation=$('#mapper-location').checked;
...
function loadMapperJob(job,duplicate=false){const config=job?.config||{},suffix=duplicate?' copy':'';$('#mapper-job-id').value=duplicate?'':job?.id||'';$('#mapper-job-name').value=(job?.name||'')+suffix;if(config.deviceID)$('#mapper-device').value=config.deviceID;setMapperWorkflow(config.mode||'discovery');$('#mapper-mode').value=config.preferredMode||'auto';if(config.startHz)$('#mapper-start').value=config.startHz/1e6;if(config.endHz)$('#mapper-end').value=config.endHz/1e6;if(config.stepHz)$('#mapper-step').value=config.stepHz/1000;if(config.dwellMilliseconds)$('#mapper-dwell').value=config.dwellMilliseconds;$('#mapper-rate').value=String(config.sampleRateHz||0);$('#mapper-concurrent').value=String(config.concurrentChannels||0);setMapperListenInput(config.decipherListenSeconds||60);$('#mapper-transcribe').checked=!!config.transcribe;$('#mapper-location').checked=!!config.includeLocation;$('#mapper-location-precision').value=config.locationPrecision||'approximate';$('#mapper-location-label').value=config.locationLabel||'';$('#mapper-latitude').value=config.latitude??'';$('#mapper-longitude').value=config.longitude??'';updateMapperWorkflow();$('#mapper-job-name').focus();}
function resetMapperJob(){loadMapperJob({config:{mode:'discovery',startHz:10e6,endHz:6e9,stepHz:12500,dwellMilliseconds:500,decipherListenSeconds:60,sampleRateHz:0,concurrentChannels:0}},false);$('#mapper-job-id').value='';}
...
for(const id of ['mapper-filter-job','mapper-filter-device','mapper-filter-type','mapper-filter-state','mapper-sort','mapper-filter-search'])$('#'+id).addEventListener(id==='mapper-filter-search'?'input':'change',renderMapper);
$('#mapper-filter-reset').addEventListener('click',()=>{for(const id of ['mapper-filter-job','mapper-filter-device','mapper-filter-type','mapper-filter-state'])$('#'+id).value='';$('#mapper-sort').value='recent';$('#mapper-filter-search').value='';renderMapper();toast('Mapper filters reset');});
