...
// dataset's exact 16-column Additions Queue schema. Users deploy it once as a
// web app, then paste its /exec URL and matching secret into GP-SDR.
const mapperAppsScript = `/** @OnlyCurrentDoc */
// GP-SDR -> Local Radio Channel Data / Additions Queue
...
var GP_SDR_SECRET = 'CHANGE_ME';
var TARGET_SHEET = 'Additions Queue';
var COLUMNS = ['dateAdded','confirmedContact','contributor','type','nameLabel','rxMHz','txMHz','mode','toneCode','locationSystem','whatWasHeard','dateTimeHeard','sourceURLFile','confidence','reviewStatus','reviewerNotes'];
...
function doPost(event) {
  ...
  try {
    ...
    const headers = sheet.getRange(4, 1, 1, 16).getDisplayValues()[0];
    const expected = ['Date Added','Confirmed Contact','Contributor','Type','Name/Label','RX MHz','TX MHz','Mode','Tone/Code','Location/System','What Was Heard','Date/Time Heard','Source URL/File','Confidence','Review Status','Reviewer Notes'];
    ...
    sheet.getRange(startRow, 1, values.length, 16).setValues(values);
